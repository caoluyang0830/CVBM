...
class Encoder(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
        ...
        # self.ffpaser = FFParser(h=112, w=112, d=41)
    ...
    def forward(self, input):
        # input = self.ffpaser(input) + input
        x1 = self.block_one(input)
        # print(x1.shape)
        # x1 = self.ffpaser(x1) + x1
        x1_dw = self.block_one_dw(x1)
        # x1_dw = self.ffpaser(x1_dw) + x1_dw
...
class CVBM(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False,
        ...
        self.decoder1_bg = Decoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, 2)
...
if __name__ == '__main__':
    ...
    model = CVBM(n_channels=1, n_classes=1, normalization='batchnorm', has_dropout=False)
